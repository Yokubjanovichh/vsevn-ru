    ...
    const DASH_NAV_TEXT_FAMILY = "'Sora', 'Roboto', Arial, sans-serif";
    ...
    function renderDashboardStaticText() {
        ...
        if (bell) {
            const lines = (bell.dataset.text || bell.textContent || '')
                ...
                .filter(Boolean);
            let bw = 0;
            lines.forEach(function (l) { bw = Math.max(bw, measureTextWidth(l, 24, 600, DASH_NAV_TEXT_FAMILY)); });
            bw += 6;
    ...
    // Шаг 1.2: верхний блок — официальные Figma-спеки (стек Sora+Roboto,
    // ls в em; многострочные тексты разделяются «|» в data-text).
    // Параграф: межстрочный шаг 26px снят с hero_block@hq.png (в Figma ~125%).
    const heroTextSpecs = [
        { selector: '.hero-title',      size: 28.99, weight: 300, color: '#747067', lineHeight: 29, ls: -0.05 },
        { selector: '.hero-subtitle',   size: 21.74, weight: 300, color: '#747067', lineHeight: 22, ls: -0.04 },
        { selector: '.hero-para',       size: 20.71, weight: 200, color: '#747067', lineHeight: 26, ls: -0.04 },
        { selector: '.hero-stat-num',   size: 49.13, weight: 300, color: '#2F3027', lineHeight: 50, ls: 0 },
        { selector: '.hero-stat-label', size: 20,    weight: 300, color: '#7B786F', lineHeight: 25, ls: 0 }
    ];
    ...
    function renderHeroStaticText() {
        heroTextSpecs.forEach(function (spec) {
            ...
            document.querySelectorAll(spec.selector).forEach(function (el) {
                ...
                el.dataset.text = raw;
                const lines = raw.split('|').map(function (s) { return s.trim(); }).filter(Boolean);
                let w = 0;
                lines.forEach(function (l) {
                    w = Math.max(w, measureTextWidth(l, spec.size, spec.weight, DASH_NAV_TEXT_FAMILY) + lsPx * Math.max(0, l.length - 1));
                });
                w += 4;
                renderElementText(el, {
                    text: lines.join(' '),
                    ...
                    size: spec.size,
                    width: w,
                    height: Math.ceil(spec.size * 0.82) + (lines.length - 1) * spec.lineHeight + Math.ceil(spec.size * 0.28),
    ...
    // Шаг 1.3: data-driven line-charts. Контейнер .line-chart несёт:
    //   data-values  — список значений через запятую
    //   data-min / data-max — диапазон шкалы Y (низ/верх области графика)
    //   data-line-frac — какую долю ширины занимает линия (0..1, default 1)
    //   data-marker="end" — вертикальная черта в конце линии
    //   data-dash="after" — горизонтальный пунктир от конца линии вправо,
    //   data-dash="first" — пунктир на уровне первого значения по всей ширине.
    // Повторный вызов initLineCharts() перерисовывает по текущим атрибутам.
    function initLineCharts() {
        document.querySelectorAll('.line-chart').forEach(function (box) {
            const svg = box.querySelector('svg');
            const raw = box.dataset.values;
            if (!svg || !raw) return;
            const values = raw.split(',').map(parseFloat).filter(Number.isFinite);
            if (values.length < 2) return;
            const vb = (svg.getAttribute('viewBox') || '0 0 100 100').split(/\s+/).map(parseFloat);
            const W = vb[2], H = vb[3];
            const vmin = parseFloat(box.dataset.min);
            const vmax = parseFloat(box.dataset.max);
            const lo = Number.isFinite(vmin) ? vmin : Math.min.apply(null, values);
            const hi = Number.isFinite(vmax) ? vmax : Math.max.apply(null, values);
            const span = (hi - lo) || 1;
            const lineFrac = Math.min(1, Math.max(0, parseFloat(box.dataset.lineFrac) || 1));
            const lineW = W * lineFrac;
            const toY = function (v) { return H - (v - lo) / span * H; };
            const pts = values.map(function (v, i) {
                return [(i / (values.length - 1)) * lineW, toY(v)];
            });
            const dLine = pts.map(function (p, i) {
                return (i ? 'L' : 'M') + p[0].toFixed(1) + ' ' + p[1].toFixed(1);
            }).join(' ');

            const line = svg.querySelector('.lc-line');
            if (line) line.setAttribute('d', dLine);

            const fill = svg.querySelector('.lc-fill');
            if (fill) fill.setAttribute('d', dLine + ' L ' + lineW.toFixed(1) + ' ' + H + ' L 0 ' + H + ' Z');

            const dash = svg.querySelector('.lc-dash');
            if (dash) {
                if (box.dataset.dash === 'first') {
                    const y = toY(values[0]).toFixed(1);
                    dash.setAttribute('d', 'M 0 ' + y + ' L ' + W + ' ' + y);
                } else {
                    const last = pts[pts.length - 1];
                    dash.setAttribute('d', 'M ' + last[0].toFixed(1) + ' ' + last[1].toFixed(1) + ' L ' + W + ' ' + last[1].toFixed(1));
                }
            }

            const marker = svg.querySelector('.lc-marker');
            if (marker) {
                marker.setAttribute('x1', lineW.toFixed(1));
                marker.setAttribute('x2', lineW.toFixed(1));
                marker.setAttribute('y1', '0');
                marker.setAttribute('y2', String(H));
            }

            // ось X: всегда по нижней кромке области графика, на всю ширину
            const axis = svg.querySelector('.lc-axis');
            if (axis) {
                const ay = (H - 1).toFixed(1);
                axis.setAttribute('x1', '0');
                axis.setAttribute('x2', String(W));
                axis.setAttribute('y1', ay);
                axis.setAttribute('y2', ay);
            }
        });
    }
    ...
    // Панель A (фикс A2): официальный шрифт Figtree (latin-only, кириллица
    // падает в Roboto с тем же весом — намеренно).
    const REPORT_TEXT_FAMILY = "'Figtree', 'Roboto', Arial, sans-serif";

    // Шаг 1.3: типографика среднего ряда. Панель A — официальные Figma-спеки
    // (fix-A2); остальные панели — замеры с Homies Lab_ru2.jpg, стек Sora+Roboto.
    const midTextSpecs = [
        { selector: '.rp-title',         size: 32.61, weight: 500, color: '#2E2F27', lineHeight: 33, family: 'figtree' },
        { selector: '.rp-num',           size: 32.61, weight: 500, color: '#2F3028', lineHeight: 33, family: 'figtree' },
        { selector: '.rp-label',         size: 15.81, weight: 300, color: '#7B7871', lineHeight: 21, family: 'figtree' },
        { selector: '.reach-title',      size: 27, weight: 300, color: '#2E2F27', lineHeight: 27, family: 'figtree' },
        { selector: '.reach-more-word',  size: 17, weight: 300, color: '#8B8880', lineHeight: 17, family: 'figtree' },
        { selector: '.reach-more-num',   size: 32.61, weight: 500, color: '#2F3028', lineHeight: 33, family: 'figtree' },
        { selector: '.reach-more-label', size: 17, weight: 300, color: '#8B8880', lineHeight: 17, family: 'figtree' },
        { selector: '.badge-text',       size: 21, weight: 400, color: '#FFFFFF', lineHeight: 21, anchor: 'middle', family: 'figtree' },
        { selector: '.reach-bar--yellow .bar-text', size: 15.5, weight: 300, color: '#FFFFFF', lineHeight: 19, anchor: 'middle', family: 'figtree' },
        { selector: '.reach-bar--black .bar-text',  size: 15.5, weight: 300, color: '#FFFFFF', lineHeight: 19, anchor: 'middle', family: 'figtree' },
        { selector: '.reach-bar--gray .bar-text',   size: 15.5, weight: 300, color: '#FFFFFF', lineHeight: 19, anchor: 'middle', family: 'figtree' },
        { selector: '.promo-num',        size: 54.5, weight: 500, color: '#2F3027', lineHeight: 55, family: 'figtree' },
        { selector: '.promo-sub',        size: 20, weight: 300, color: '#7C7971', lineHeight: 20, family: 'figtree' },
        { selector: '.promo-month',      size: 22, weight: 400, color: '#2F3027', lineHeight: 22, anchor: 'middle', family: 'figtree' },
        { selector: '.month-label',      size: 25, weight: 400, color: '#7C7971', lineHeight: 25, anchor: 'middle', family: 'figtree' },
        { selector: '.month-pill',       size: 25, weight: 400, color: '#CCA82D', lineHeight: 25, anchor: 'middle', family: 'figtree' },
        // Карточки «Посмотреть» — официальные Figma-спеки (fix-V): стек Sora
        // (кириллица намеренно падает в Roboto). lineHeight 14.4 — шаг
        // baseline'ов эталона vcard@4x (55.5 -> 69.9).
        { selector: '.vcard-title',      size: 12.09, weight: 300, color: '#2E2F27', lineHeight: 14.4 },
        { selector: '.vcard-num',        size: 26.81, weight: 400, color: '#2E2F27', lineHeight: 27 },
        { selector: '.vcard-view',       size: 12.09, weight: 300, color: '#2E2F27', lineHeight: 13 }
    ];

    function renderMidStaticText() {
        midTextSpecs.forEach(function (spec) {
            const family = spec.family === 'figtree' ? REPORT_TEXT_FAMILY : DASH_NAV_TEXT_FAMILY;
            document.querySelectorAll(spec.selector).forEach(function (el) {
                const raw = el.dataset.text || el.textContent.trim();
                if (!raw) return;
                el.dataset.text = raw;
                const lines = raw.split('|').map(function (t) { return t.trim(); }).filter(Boolean);
                let w = 0;
                lines.forEach(function (l) {
    ...
    // ========================================================================
    // Шаг 1.4: таблица «Подобранные резюме соискателей» — data-driven.
    // Строки рендерятся из RESUME_ROWS (backend-ready). Замеры ru2:
    // x ячеек (панель-относительно), кегли по колонкам; baseline строки 50.5
    // от верха ряда (шаг рядов 89, G2 — вертикальная центровка по строчным).
    // R4: две строки с одним счётом — №3 Васильев и №12 Морозов.
    // ========================================================================
    // Кегли (фикс-cells, финальный вердикт): ФИО/телефон 23, ВАКАНСИЯ и
    // ПОЧТА — 20 (замер ru2; спек 23 был из другого контекста), даты 20,
    // счёт 18.82, всё 300. x ячеек — замеры ru2 (панель-относительно).
    // maxW — защитная граница: до следующей колонки минус ~16 (дизайн без
    // truncation; просто страховка от вылезания длинных данных).
    const RESUME_COLS = [
        { key: 'name',        x: 49,     size: 23,    maxW: 257.8 },
        { key: 'phone',       x: 322.8,  size: 23,    maxW: 201.4 },
        { key: 'vacancy',     x: 540.2,  size: 20,    maxW: 340 },
        { key: 'email',       x: 896.2,  size: 20,    maxW: 268.8 },
        { key: 'replyDate',   x: 1181,   size: 20,    maxW: 176.5 },
        { key: 'account',     x: 1373.5, size: 18.82, maxW: 231.5 },
        { key: 'accountDate', x: 1621,   size: 20,    maxW: 169 }
    ];

    const RESUME_ROWS = [
        { name: 'Антонов Дмитрий',    phone: '89213451782', vacancy: 'Менеджер по продажам', email: 'antonov.d@mail.ru',        replyDate: '11 Нояб 2024', account: 'Св000000001811', accountDate: '11 Апреля 2024' },
        { name: 'Борисова Анна',      phone: '89052184431', vacancy: 'Бухгалтер',            email: 'borisova.anna@yandex.ru',  replyDate: '14 Нояб 2024', account: 'Св000000001814', accountDate: '15 Апреля 2024' },
        { name: 'Васильев Пётр',      phone: '89126339047', vacancy: 'Водитель',             email: 'vasilev.pyotr@mail.ru',    replyDate: '18 Нояб 2024', account: 'Св000000001829', accountDate: '21 Апреля 2024' },
        { name: 'Григорьева Ольга',   phone: '89264702215', vacancy: 'Уборщица',             email: 'grigoreva.o@gmail.com',    replyDate: '21 Нояб 2024', account: 'Св000000001831', accountDate: '24 Апреля 2024' },
        { name: 'Дмитриев Сергей',    phone: '89037815609', vacancy: 'Электрик',             email: 'dmitriev.s@yandex.ru',     replyDate: '25 Нояб 2024', account: 'Св000000001840', accountDate: '28 Апреля 2024' },
        { name: 'Егорова Мария',      phone: '89172647853', vacancy: 'Повар',                email: 'egorova.m@mail.ru',        replyDate: '28 Нояб 2024', account: 'Св000000001846', accountDate: '5 Мая 2024' },
        { name: 'Жуков Андрей',       phone: '89093524176', vacancy: 'Сварщик',              email: 'zhukov.a@gmail.com',       replyDate: '2 Дек 2024',   account: 'Св000000001852', accountDate: '12 Мая 2024' },
        { name: 'Захаров Константин', phone: '89527778888', vacancy: 'Грузчик',              email: 'zakharov.k@mail.ru',       replyDate: '5 Дек 2024',   account: 'Св000000001857', accountDate: '19 Мая 2024' },
        { name: 'Иванова Дарья',      phone: '89315081264', vacancy: 'Кассир',               email: 'ivanova.darya@yandex.ru',  replyDate: '9 Дек 2024',   account: 'Св000000001863', accountDate: '26 Мая 2024' },
        { name: 'Ковалёв Игорь',      phone: '89159403528', vacancy: 'Охранник',             email: 'kovalev.igor@mail.ru',     replyDate: '12 Дек 2024',  account: 'Св000000001870', accountDate: '2 Июня 2024' },
        { name: 'Лебедева Наталья',   phone: '89241165990', vacancy: 'Администратор',        email: 'lebedeva.n@gmail.com',     replyDate: '16 Дек 2024',  account: 'Св000000001874', accountDate: '9 Июня 2024' },
        { name: 'Морозов Алексей',    phone: '89086278342', vacancy: 'Дворник',              email: 'morozov.a@yandex.ru',      replyDate: '19 Дек 2024',  account: 'Св000000001829', accountDate: '21 Апреля 2024' },
        { name: 'Никитина Елена',     phone: '89194732061', vacancy: 'Токарь',               email: 'nikitina.elena@mail.ru',   replyDate: '23 Дек 2024',  account: 'Св000000001885', accountDate: '23 Июня 2024' },
        { name: 'Орлов Виктор',       phone: '89278059417', vacancy: 'Жестянщик',            email: 'orlov.viktor@gmail.com',   replyDate: '26 Дек 2024',  account: 'Св000000001891', accountDate: '30 Июня 2024' },
        { name: 'Петрова Светлана',   phone: '89113826705', vacancy: 'Фармацевт',            email: 'petrova.s@yandex.ru',      replyDate: '30 Дек 2024',  account: 'Св000000001898', accountDate: '7 Июля 2024' },
        { name: 'Романов Павел',      phone: '89255493178', vacancy: 'Разнорабочий',         email: 'romanov.pavel@mail.ru',    replyDate: '3 Окт 2024',   account: 'Св000000001902', accountDate: '14 Июля 2024' },
        { name: 'Соколова Ирина',     phone: '89067182549', vacancy: 'Швея',                 email: 'sokolova.irina@yandex.ru', replyDate: '7 Окт 2024',   account: 'Св000000001910', accountDate: '21 Июля 2024' },
        { name: 'Тихонов Олег',       phone: '89132708634', vacancy: 'Логист',               email: 'tikhonov.o@gmail.com',  replyDate: '14 Окт 2024',  account: 'Св000000001917', accountDate: '28 Июля 2024' },
        { name: 'Фёдорова Ксения',    phone: '89294617290', vacancy: 'Инженер',              email: 'fedorova.k@mail.ru',       replyDate: '21 Окт 2024',  account: 'Св000000001923', accountDate: '4 Августа 2024' },
        { name: 'Харитонов Глеб',     phone: '89028945013', vacancy: 'Зоотехник',            email: 'kharitonov.g@yandex.ru',   replyDate: '28 Окт 2024',  account: 'Св000000001930', accountDate: '11 Августа 2024' }
    ];

    const RESUME_ROW_BASELINE = 50.5;   // baseline текста от верха ряда (ru2)
    const RESUME_TEXT_COLOR = '#2E2F27';

    function renderResumeCellText(el, text, size) {
        const w = measureTextWidth(text, size, 300, DASH_NAV_TEXT_FAMILY) + 4;
        renderElementText(el, {
            text: text,
            size: size,
            width: w,
    ...
    function initResumeTable() {
        ...
        if (box.children.length !== RESUME_ROWS.length) {
            box.textContent = '';
            RESUME_ROWS.forEach(function (row) {
                const tr = document.createElement('div');
                tr.className = 'resume-row';
                RESUME_COLS.forEach(function (col) {
                    const cell = document.createElement('span');
                    cell.className = 'resume-cell cell-' + col.key;
                    cell.style.left = 'calc(' + col.x + ' * var(--dpx, 0.0520833vw))';
                    cell.style.top = 'calc(' + (RESUME_ROW_BASELINE - Math.round(col.size * 0.82)) +
                        ' * var(--dpx, 0.0520833vw))';
    ...
    // ========================================================================
    // Шаг 1.5: поиск по таблице (R1/R2/R3/R5, P6, G6).
    // Префикс-поиск ТОЛЬКО с начала слова (ФИО + ВАКАНСИЯ), без учёта
    // регистра; совпадение подсвечивается оранжевой плашкой #FD6429 с белым
    // текстом (эталон ref/image11.png, масштаб 1:1: плашка cap-top−5 …
    // baseline+5, r4 при кегле 23 — масштабируется k=size/23).
    // ========================================================================
    const RESUME_HL_BG = '#FD6429';
    const RESUME_HL_FG = '#FFFFFF';

    function wordPrefixRanges(text, query) {
        const ranges = [];
        if (!query) return ranges;
        const q = String(query).toLowerCase();
        const re = /[0-9A-Za-zЀ-ӿ]+/g;
        let m;
        while ((m = re.exec(String(text))) !== null) {
            if (m[0].toLowerCase().indexOf(q) === 0) {
                ranges.push({ start: m.index, end: m.index + q.length });
            }
        }
        return ranges;
    }
    ...
    function renderHighlightedCellText(el, text, size, ranges) {
        const family = DASH_NAV_TEXT_FAMILY;
        const w = measureTextWidth(text, size, 300, family) + 4;
        const baseOpts = {
            text: text,
            size: size,
            width: w,
        ...
        const key = getSvgTextRenderKey(text, baseOpts) + 'hl:' +
            ranges.map(function (r) { return r.start + '-' + r.end; }).join(',');
        if (el.dataset.svgTextKey === key && el.firstElementChild) return;

        const svg = createSvgText(text, baseOpts);
        const label = svg.querySelector('text');
        label.style.whiteSpace = 'pre'; // не схлопывать пробелы на границах tspan
        label.textContent = '';

        const k = size / 23;
        const baseline = baseOpts.y;
        const rectY = baseline - 0.711 * size - 5 * k;
        const rectH = (baseline + 5 * k) - rectY;
        const clean = function (s) {
            return s ? measureTextWidth(s, size, 300, family) - 2 : 0;
        };
        ...
        ranges.forEach(function (r) {
            ...
            rect.setAttribute('width', (clean(text.slice(r.start, r.end)) + 6 * k).toFixed(2));
        ...
        let cursor = 0;
        ranges.forEach(function (r) {
            if (r.start > cursor) addSpan(text.slice(cursor, r.start), RESUME_TEXT_COLOR);
            addSpan(text.slice(r.start, r.end), RESUME_HL_FG);
            cursor = r.end;
        });
        if (cursor < text.length) addSpan(text.slice(cursor), RESUME_TEXT_COLOR);
    ...
    function resumeColSize(key) {
        for (let i = 0; i < RESUME_COLS.length; i++) {
            if (RESUME_COLS[i].key === key) return RESUME_COLS[i].size;
        }
        return 23;
    }
    ...
    function applyResumeSearch() {
        ...
        box.querySelectorAll('.resume-row').forEach(function (tr, i) {
            ...
            if (nameCell) {
                if (nameR.length) renderHighlightedCellText(nameCell, row.name, resumeColSize('name'), nameR);
                else renderResumeCellText(nameCell, row.name, resumeColSize('name'));
            }
            if (vacCell) {
                if (vacR.length) renderHighlightedCellText(vacCell, row.vacancy, resumeColSize('vacancy'), vacR);
        ...
        // чтобы попап никогда не обрезался снизу
        const popup = document.getElementById('accPopup');
        if (popup && !popup.hidden) ensureAccPopupVisible(popup);
    }

    // R5: фокус/значение — лупа и плейсхолдер скрываются (CSS-классы),
    // каретка встаёт на место лупы. P6: ✕ очищает поле и фильтр.
    let resumeSearchBound = false;
    function initResumeSearch() {
        const wrap = document.querySelector('.resume-search');
        if (!wrap) return;
        const input = wrap.querySelector('.resume-search-input');
        const clearBtn = wrap.querySelector('.resume-clear-btn');
        if (!input) return;
        if (!resumeSearchBound) {
            resumeSearchBound = true;
    ...
    const ACCOUNTS = [
        ...
        { num: 'Св000000001930', date: '11.08.2024' },
        { num: 'Св000000001923', date: '04.08.2024' },
        { num: 'Св000000001917', date: '28.07.2024' },
        { num: 'Св000000001910', date: '21.07.2024' },
        { num: 'Св000000001902', date: '14.07.2024' },
        { num: 'Св000000001898', date: '07.07.2024' },
        { num: 'Св000000001891', date: '30.06.2024' },
        { num: 'Св000000001885', date: '23.06.2024' },
        { num: 'Св000000001874', date: '09.06.2024' },
        { num: 'Св000000001870', date: '02.06.2024' },
        { num: 'Св000000001863', date: '26.05.2024' },
        { num: 'Св000000001857', date: '19.05.2024' },
        { num: 'Св000000001852', date: '12.05.2024' },
        { num: 'Св000000001846', date: '05.05.2024' },
        { num: 'Св000000001840', date: '28.04.2024' },
        { num: 'Св000000001831', date: '24.04.2024' },
        { num: 'Св000000001829', date: '21.04.2024' },
        { num: 'Св000000001814', date: '15.04.2024' },
        { num: 'Св000000001811', date: '11.04.2024' }
    ];

    function accDateKey(date) {
        const p = String(date).split('.');
        return Number(p[2]) * 10000 + Number(p[1]) * 100 + Number(p[0]);
    }

    // POP3: место вставки в убывающем по дате списке (не ломая порядок)
    function accountInsertIndex(list, entry) {
        const k = accDateKey(entry.date);
        for (let i = 0; i < list.length; i++) {
            if (k >= accDateKey(list[i].date)) return i;
        }
        return list.length;
    }
    ...
    function buildAccList(list) {
        ...
        ACCOUNTS.forEach(function (acc) {
            ...
            cb.className = 'acc-cb';
            cb.innerHTML = '<svg viewBox="0 0 26.7 26.7" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M6.5 13.5l5 5 8.5-10" stroke="#FFFFFF" stroke-width="2.6" ' +
                'stroke-linecap="round" stroke-linejoin="round"/></svg>';
            const num = document.createElement('span');
            num.className = 'acc-num';
            num.dataset.text = 'Счёт №' + acc.num;
            const date = document.createElement('span');
            date.className = 'acc-date';
            date.dataset.text = 'от ' + acc.date;
    ...
    // счета длиннее, чем в sheet): дата начинается после САМОГО длинного
    // номера + зазор 14.2; ширина вмещает и заголовок с полями 14.2.
    function layoutAccPopup(popup) {
        const titleEl = popup.querySelector('.acc-popup-title');
        const titleW = titleEl
            ? measureTextWidth(titleEl.dataset.text, 22, 300, DASH_NAV_TEXT_FAMILY)
            : 540;
        let maxNum = 0;
        ACCOUNTS.forEach(function (a) {
            maxNum = Math.max(maxNum, measureTextWidth('Счёт №' + a.num, 20, 300, DASH_NAV_TEXT_FAMILY));
        });
        let maxDate = 0;
        ACCOUNTS.forEach(function (a) {
            maxDate = Math.max(maxDate, measureTextWidth('от ' + a.date, 20, 300, DASH_NAV_TEXT_FAMILY));
        });
        const dateX = 51.6 + maxNum + 12;
        const w = Math.max(14.2 + titleW + 14.2, dateX + maxDate + 14.2 + 34);
        popup.style.setProperty('--acc-w', w.toFixed(1));
    ...
    function renderAccPopupTexts() {
        document.querySelectorAll('.acc-popup-title').forEach(function (el) {
            renderElementText(el, {
                ...
                size: 22,
                width: measureTextWidth(el.dataset.text, 22, 300, DASH_NAV_TEXT_FAMILY) + 4,
                height: 26,
                y: Math.round(22 * 0.82),
                weight: 300,
                color: '#584E4E',
        ...
        document.querySelectorAll('.acc-num, .acc-date').forEach(function (el) {
            renderElementText(el, {
                text: el.dataset.text,
                size: 20,
                width: measureTextWidth(el.dataset.text, 20, 300, DASH_NAV_TEXT_FAMILY) + 4,
                height: 23,
    ...
    // внутри, страница растёт вместе с картой).
    function computePopupReveal(popupBottomDoc, cardBottomDoc, viewportH, scrollY) {
        const margin = 12;
        const need = popupBottomDoc + margin;
        const spacer = Math.max(0, need - cardBottomDoc);
        const scrollBy = Math.max(0, (need - viewportH) - scrollY);
        return { spacer: spacer, scrollBy: scrollBy };
    }

    function removePageSpacer() {
        const sp = document.getElementById('pageSpacer');
        if (sp) sp.remove();
    }

    function ensureAccPopupVisible(popup) {
        removePageSpacer();
        const r = popup.getBoundingClientRect();
        const card = document.querySelector('.dash-card');
        const cardBottom = card
            ? card.getBoundingClientRect().bottom + window.scrollY
            : document.documentElement.scrollHeight;
        const res = computePopupReveal(
        ...
        if (res.spacer > 0) {
            ...
            sp.id = 'pageSpacer';
            sp.style.height = res.spacer + 'px';
            (card || document.body).append(sp);
        }
    ...
    // ========================================================================
    // Шаг 1.8-A: планшетное представление (<800px). Размеры макета image5
    // (852px) переводятся в dpx коэффициентом TAB_T — единая vw-система.
    ...
    function renderTabText(el, makSize, weight, color, makLh, koef) {
        ...
        const raw = (T === PHONE_T && el.dataset.textPhone) ? el.dataset.textPhone : el.dataset.text;
        if (!raw) return;
        ...
        renderElementText(el, {
            ...
            size: size,
            width: w + 4,
            height: Math.ceil(size * 0.82) + (lines.length - 1) * lh + Math.ceil(size * 0.28),
            y: Math.round(size * 0.82),
            weight: weight,
            color: color,
            family: DASH_NAV_TEXT_FAMILY,
    ...
    function initTabView() {
        ...
        // телефон (1.8-B) — image7 (~400-баз. макет-px), вакансия КРАСНАЯ #850408
        ...
        if (box.children.length !== RESUME_ROWS.length) {
            ...
            RESUME_ROWS.forEach(function (row) {
                ...
                left.className = 'trow-left';
                const name = document.createElement('span');
                name.className = 'trow-name';
                name.dataset.text = row.name;
                const phone = document.createElement('span');
                phone.className = 'trow-phone';
                phone.dataset.text = formatPhoneIntl(row.phone);
                left.append(name, phone);
                const vac = document.createElement('span');
                vac.className = 'trow-vac';
                vac.dataset.text = row.vacancy;
                tr.append(left, vac);
                box.append(tr);
            });
        }
    ...
    function initScrollTop() {
        ...
        if (txt) {
            renderElementText(txt, {
                ...
                size: 24,
                width: measureTextWidth(txt.dataset.text, 24, 400, DASH_NAV_TEXT_FAMILY) + 4,
                height: 27,
    ...
    // fix-4: генерик горизонтальный clamp тултипов — при hover плашка
    // замеряется и, если выходит за контент карты/вьюпорт, сдвигается влево
    // (right-edge align, тянется влево — как «Деактивировать…» в sheet);
    // симметрично для левого края. За мышью не следует, gap не меняется.
    let uiTipClampBound = false;
    function initUiTipClamp() {
        if (uiTipClampBound) return;
        uiTipClampBound = true;
        document.addEventListener('mouseover', function (e) {
            const owner = e.target.closest && e.target.closest('.has-ui-tip');
            if (!owner) return;
            const tip = owner.querySelector('.ui-tip');
            if (!tip) return;
            ...
            const pad = 8;
            const rightEdge = Math.min(cardRect ? cardRect.right : Infinity, window.innerWidth) - pad;
            const leftEdge = Math.max(cardRect ? cardRect.left : 0, 0) + pad;
            let shift = 0;
            if (r.right > rightEdge) shift = rightEdge - r.right;
            if (r.left + shift < leftEdge) shift = leftEdge - r.left;
            if (shift) tip.style.marginLeft = shift.toFixed(1) + 'px';
        });
    }
    ...
    function gaugePoint(deg) {
        ...
        return {
            x: GAUGE_GEOMETRY.cx + GAUGE_GEOMETRY.r * Math.cos(rad),
            y: GAUGE_GEOMETRY.cy + GAUGE_GEOMETRY.r * Math.sin(rad)
        };
    }

    function gaugeArcPath(fromDeg, toDeg) {
        const a = gaugePoint(fromDeg);
        const b = gaugePoint(toDeg);
        const large = (toDeg - fromDeg) > 180 ? 1 : 0;
        return 'M ' + a.x.toFixed(1) + ' ' + a.y.toFixed(1) +
            ' A ' + GAUGE_GEOMETRY.r + ' ' + GAUGE_GEOMETRY.r + ' 0 ' + large + ' 1 ' +
            b.x.toFixed(1) + ' ' + b.y.toFixed(1);
    }

    function initGauge() {
        document.querySelectorAll('.hero-gauge').forEach(function (box) {
            const svg = box.querySelector('svg');
            if (!svg) return;
            const value = parseFloat(box.dataset.value);
            const max = parseFloat(box.dataset.max) || 100;
            if (!Number.isFinite(value)) return;
            const ratio = Math.max(0, Math.min(1, value / max));
            const knobDeg = GAUGE_GEOMETRY.startDeg + ratio * (GAUGE_GEOMETRY.endDeg - GAUGE_GEOMETRY.startDeg);
            ...
            if (progress) {
                ...
                if (ratio > 0) progress.setAttribute('d', gaugeArcPath(GAUGE_GEOMETRY.startDeg, knobDeg));
            }
            if (track) {
                track.style.display = ratio < 1 ? '' : 'none';
                if (ratio < 1) track.setAttribute('d', gaugeArcPath(knobDeg, GAUGE_GEOMETRY.endDeg));
            }
            ...
            if (prefixText) {
                ...
                const valueWidth = measureTextWidth(valueStr, GAUGE_GEOMETRY.valueSize, GAUGE_GEOMETRY.valueWeight, DASH_NAV_TEXT_FAMILY);
                prefixText.setAttribute('x', (GAUGE_GEOMETRY.cx - valueWidth / 2 - GAUGE_GEOMETRY.prefixGap).toFixed(1));
            }
        });
    }
